...
//
//prikaz opisa parametrov, ko pride do mouseover info slike
//
$(document).ready(function() {
	...
	//opis candock input parametrov
	var info_text = ["help gradient tolerance", "help d steps", "help ABNR", "help solvent"];
	
	// modeling parameters
	$("#info_0, #info_1, #info_2, #info_3 ").hover(function(e) {
		
		$(".info").html(info_text[this.id.substring(5)]);
   	$(".info").css({top: eval(dimY+20)+"px", left: eval(dimX+20)+"px"}).show();
	...
	// focus on input	
/*	$("#tolg, #sdstep, #abnrstep").hover(function(e) {
		
		if (this.id == "tolg") {
			$('#info_0').trigger(e.type);
		}
		else if (this.id == "sdstep") {
			$('#info_1').trigger(e.type);
		}
		else {
			$('#info_2').trigger(e.type);		
		}		
	});*/
	...
	// search okno
	$( "#button-probisdb" ).click(function( event ) {
  		event.preventDefault();
  		var url = window.location.origin+"/probis-server/?q_id="+$( "#search-probisdb" ).val();
...
//
// naredi barvno legendo
//
var set_color_range = function(barva, protein) {

	$(".clicked").removeClass("clicked");
	
	if (barva == "colors_bgyr") {
		show_color_range();	
		for (i=1; i<=10; i++){
			$("#color"+i).css('background-color', colors_bgyr[i-1]);
		}
	}	
	else if (barva == "colors_br") {
		show_color_range();	
		for (i=1; i<=10; i++){
			$("#color"+i).css('background-color', colors_br[i-1]);
		}
	}
	else if (barva == "colors_wg") {
		show_color_range();
		for (i=1; i<=10; i++){
			$("#color"+i).css('background-color', colors_wg[i-1]);
		}
	}
	else if (barva == "colors_wb") {
		show_color_range();	
		for (i=1; i<=10; i++){
			$("#color"+i).css('background-color', colors_wb[i-1]);
		}
	}
	else {
		hide_color_range();
	...
	// set color legend
	if (barva == "colors_alrc") {
		show_color_legend();
		$(".cl_color1").css('background-color', "cyan");
		$(".cl_color2").css('background-color', "red");
		set_protein_labels(protein);
	}	
	else if (barva == "colors_algb") {
		show_color_legend();	
		$(".cl_color1").css('background-color', "blue");
		$(".cl_color2").css('background-color', "green");
		set_protein_labels(protein);
	}
	else {
		hide_color_legend();
...
$(document).ready(function() {
	...
	$( ".color_range" ).bind( "click", function() {
  		var myClass = $(this).attr("class").split(" ");
  		//var idx = myClass.indexOf("color_range");
  		
  		var color = parseInt($(this).attr("id").replace("color", "")); 
  		...
  		$(".clicked").each(function() {
  			var idx = parseInt($(this).attr("id").replace("color", ""));
  			var from = (idx-1)*0.1;
  			var to = (idx*0.1);
  			selectedCommands.push("(temperature>="+from.toString()+" and temperature<"+to.toString()+")");
		});

  		var selectedCommand = "select ";
  		for (var i=0; i < selectedCommands.length; i++) {
  			if (i==0) {
  				selectedCommand += selectedCommands[i];
  			}
  			else {
  				selectedCommand += " or "+selectedCommands[i];
  			}
  		}
  		...
  		jml.select_temp_range(selectedCommand);

	});



 	$('body').on('click', 'img.table_image', function() {

 		var id = jQuery(this).attr("id").split("_")[1];

 		if ($('.alignment_ozadje').is(":visible") == false ) {
			$(".alignment_ozadje").draggable({ handle: ".alignment_header" });
			$('.alignment_ozadje').draggable('enable');
			$('.alignment_ozadje').show('slow');
		}

 		$("#alignment_residues tr").remove(); 

 		var tmp = parseInt(id) + 1;
 		var selectedAlignment = $("#alignment_num_"+id).val();
 		if (!selectedAlignment) {
 			selectedAlignment = 0;
 		}

 		$(".alignment_htxt").text("Alignment Rank "+tmp.toString()+" #"+selectedAlignment.toString());
 		

		var columnsContent = "<tr bgcolor='black' style='color:white;'><th colspan='2'>"+__qpdb.toUpperCase()+", "+__qcid+"</th> \
								<th colspan='2'>"+jdat[id].pdb_id.toUpperCase()+", "+jdat[id].chain_id+"</th></tr>";

    	$("#alignment_residues tbody").append(columnsContent);

		columnsContent = "<tr bgcolor='#3498db'><th>Residue Name</th><th>Pos.</th> \
						<th>Residue Name</th><th>Pos.</th></tr>";

    	$("#alignment_residues tbody").append(columnsContent);
		
		var alignedResidues = jdat[id].alignment[selectedAlignment].aligned_residues;
		alignedResidues.forEach(function(residue) {
			var tmp = residue.a.split(/:|=/);
			var newRow = "<tr align='center' ><td>"+aminoAcids[tmp[0].toUpperCase()]+"</td>\
							  <td>"+tmp[1]+"</td>\
							  <td>"+aminoAcids[tmp[3].toUpperCase()]+"</td>\
							  <td>"+tmp[4]+"</td>\
							  </tr>";
				$("#alignment_residues tbody").append(newRow);
		});

		alignData = undefined;
		alignedResidues = undefined;

	});
	

	$('#alignment_close').click(function() {
		$('.alignment_ozadje').hide();
		$('.alignment_ozadje').draggable('disable');
	});


	$("#alignment_residues").on("click", "tr", function() {
       	$(this).addClass('selected_align').siblings().removeClass('selected_align'); 
       	var value=$(this).find('td:first').html();
	});


	$('#alignment_save').click(function() {

		downloadTable("downloadTable.php", "probis_align_residues.csv", get_alignment_table_data("\t","\r\n"));

	});

	$('#alignment_copy').click(function() {

		var tableData = get_alignment_table_data(" ", "\r\n");
		...
		if (msg == "unsuccessful") {
		    window.prompt("Copy to clipboard: Ctrl+C", tableData);
		}
...
var get_alignment_table_data = function(tab, n) {

	var dataExport="";
		
	var table = document.getElementById('alignment_residues');
	...
	for (i = 0; i < rowLength; i++){
          ...
          dataExport += cellVal+tab;
        }
        dataExport = dataExport.slice(0,-1) + n;
    }
...
var set_protein_labels = function(protein) {

	var names = protein.split('/');
	if (names.length > 2){
		$("#cl_protein1").text(names[0]+"/"+names[2]);
		$("#cl_protein2").text(names[1]+"/"+names[2]);
	}
	else {
		$("#cl_protein1").text(names[0]);
		$("#cl_protein2").text(names[1]);
	}
	
}

//
// prikaže barvno lestvico
//

var show_color_range = function() {
	
	if ($("#color_range").css('display') == "none") {
		$("#color_range").show();
		var h = $("#jmlfun").height();
		$("#jmlfun").css({ 'height': eval(h-20)+"px" });
		$("#j1Probis .fill-horizontal").css({ 'height': "55px" });
	}
		
};

//
// skrije barvno lestvico
//

var hide_color_range = function() {
	
	if ($("#color_range").css('display') != "none") {
		$("#color_range").hide();
		var h = $("#jmlfun").height();
		$("#jmlfun").css({ 'height': eval(h+20)+"px" });
		$("#j1Probis .fill-horizontal").css({ 'height': "25px" });
	}
...
//
// prikaže alignment barvno legendo
//

var show_color_legend = function() {
	
	if ($("#color_legend").css('display') == "none") {
		$("#color_legend").show();
		var h = $("#jmlfun").height();
		$("#jmlfun").css({ 'height': eval(h-20)+"px" });
		$("#j1Probis .fill-horizontal").css({ 'height': "55px" });
...
//
// skrije alignment barvno legendo
//

var hide_color_legend = function() {
...
//
// hide/show desno stran
//
$(document).ready(function () {
    
    $('#hide_tab').click(function () {
       if ($(".right-floated").is(':visible')) {
       	$('.right-floated').hide("slide", { direction: "right" }, 300);
       	//$('#hide_tab').attr("src","slike/show-icon.png");
       	if ($('.bckgrnd').html() == "White Background") {
				$("#tab_btns").css("background-color","white");		
			}  
       	$('#hide_tab').attr("title","show table");
       }
       else {
       	$('.right-floated').show("slide", { direction: "right" }, 300);
       	//$('#hide_tab').attr("src","slike/hide-icon.png");
       	if ($('.bckgrnd').html() == "White Background") {
				$("#tab_btns").css("background-color","transparent");		
			}
       	$('#hide_tab').attr("title","hide table");
       }
       setTimeout(function() {
      	resize_results_page();
      	$(window).resize();
...
var set_btns = function () {

	var w = $(".fill-horizontal").width();
	var w2 = $(".jmiddle-flex").width();
	//alert(w+" "+w2);
	
	$(".jmiddle-flex").css({"margin-left": eval(w/2-270)+"px"});
...
//
// F-ja, ki raztegne vse dead-center (in ostale) elemente na celotno sirino browserja.
//
/*
var resize_results_page = function() {
	var width = Number($(window).width());
	if (width < 1044) width = 1044;
	if (width > 1920) width = 1920;
	$('.dead-center').css({'width': String(width - 20) + 'px','margin-left': '-' + String((width - 20)/ 2) + 'px'});
	//~ $('#draggable').css({'width': String(300) + 'px','margin-left': '-' + String((width - 20)/ 2) + 'px'});
	if ( $(".right-floated").is(':visible') ) {
		$('.left-floated, .right-floated').css({'width': String(Math.round((width - 53) / 2)) + 'px'});
	}
	else {
		$('.left-floated').css({'width': String(Math.round((width - 53))) + 'px'});
	}	
	$('#search-probisdb').css({'width': String(Math.round(width*0.25)) + 'px'});
	//$('#content_color').css({'margin-left: '})
};*/
//
// Preverimo ali je prisotna Java, in ce je nalozimo Jmol appet, ce ni nalozimo staticno sliko.
//
$(document).ready(function() {
	setTimeout(function() {
...
//
// Povecamo/zmanjsamo stran in repozicioniramo slike binding siteov/ligandov, ko se sprozi window.resize event.
//
$(window).resize(function() {
	resize_results_page();
	resize_tabs("#list4");
	resize_tabs("[id^='table-" + jml.type._ligand.protein  + "']");
	resize_tabs("[id^='table-" + jml.type._ligand.nucleic  + "']");
	resize_tabs("[id^='table-" + jml.type._ligand.hetero + "']");
	resize_tabs("[id^='table-" + jml.type._ligand.ion      + "']");
...
//
// Spremenimo iz formata ABC v format <span class='chainA'>A</span><span
// class='chainB'>B</span><span class='chainC'>C</span>.
//
function convert_chain_id(chain_id) {
	c_id = "<span class='chain" + chain_id[0] + "'>" + chain_id[0] + "</span>";
	for (i = 1; i < chain_id.length; i++) {
		c_id += "<span class='chain" + chain_id[i] + "'>" + chain_id[i]
				+ "</span>";
	}
	return c_id;
}
...
//
// Inicializiramo nekatere spremenljivke.
//
var warn_jdat_empty = function() {
		// ce ni niti enega prileganega pdb-ja, potem izpisemo, da nismo nasli nic 
		if (jdat.length == 0) {
			alert("No local structural alignments were found! Set the Cutoff Z-score to lower value (0 or -1) to get more alignments.");
		}
};
...
var resize_results_page = function() {
	...
	if ( $(".right-floated").is(':visible') ) {
		...
		$('.left-floated').css({'width': String(Math.round((width - 42))) + 'px'});
	}	
	set_btns();

	var w = $(window).width();
	if (w < 768) {
		$('.custom-navbar').css("margin-left", "20px");
	}
	else {
		var w_offset = eval($('.navbar-collapse').width() - $('.custom-navbar').width())/2;
		$('.custom-navbar').css("margin-left", w_offset+"px");
	}
};
